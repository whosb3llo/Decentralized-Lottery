...
2. **🎫 Buy Tickets**: Players call `buy-ticket` or `buy-multiple-tickets`
3. **⏳ Wait for Minimum**: Need at least 10 tickets sold
4. **🎲 Draw Winner**: Owner calls `draw-winner` (uses block hash for randomness)
5. **💰 Claim Prize**: Winner calls `claim-prize` to receive 80% of prize pool
6. **🔄 Repeat**: Start new lottery for next round
